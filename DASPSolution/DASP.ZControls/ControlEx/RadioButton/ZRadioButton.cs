...
namespace DASP.ZControls
{
    ...
    public class ZRadioButton : RadioButton
    {
        ...
        private Image _dotImg = RenderHelper.GetImageFormResourceStream("ControlEx.RadioButton.Image.dot.png");
        ...
        [Description("获取ZRadioButton左边正方形的宽度")]
        ...
        private void DrawNormalCircle(Graphics g, Rectangle circleRect)
        {
            ...
            using (Pen borderPen = new Pen(ColorTable.BorderColor))
            {
        ...
        {
            DrawNormalCircle(g, circleRect);
            using (Pen p = new Pen(ColorTable.HighLightInnerColor))
            {
                g.DrawEllipse(p, circleRect);

                circleRect.Inflate(1, 1);
                p.Color = ColorTable.HighLightColor;
